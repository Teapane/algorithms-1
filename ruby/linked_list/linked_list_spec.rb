...
describe LinkedList do
  ...
  it "can add an element after an index" do
    list = LinkedList.new
    expect(list.insert_after(5, 'good day')).to be_false
    node1 = list.push("howdy")
    node2 = list.push("hello")
    node3 = list.push("heyo")
    expect(list.insert_after(5, "good day")).to be_false
    expect(list.count).to eq(3)
    node4 = list.insert_after(2, "good day")
    expect(list.count).to eq(4)
    expect(node1.link.data).to eq('hello')
    expect(node4.link.data).to eq('heyo')
  end
